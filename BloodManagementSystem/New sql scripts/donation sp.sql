...
--SEARCH
ALTER PROCEDURE spLOAD_SEARCH_DONOR_PER_CHECK
@ID INT
AS
BEGIN
        SELECT * FROM DONOR_PERCHECK WHERE ID = @ID
END
...
END
-- DISPLAY SINGLE
CREATE PROCEDURE spLOAD_FULL_CHECK
@id int
AS
BEGIN
...
CREATE PROCEDURE spDECREMENT_BLOODCOUNT
@value int,
@type varchar(3)
AS
BEGIN
...
CREATE PROCEDURE sp_DISPLAY_ILLNESSINFO
AS
BEGIN
	SELECT * from ILLNESS_INFO;
END

CREATE PROCEDURE sp_searchHospByName
@Name varchar(100)
AS
BEGIN
...
CREATE PROCEDURE SecondRequest
@id int
AS
BEGIN
	select count(*) from FULL_CHECK where ID = @id;
END